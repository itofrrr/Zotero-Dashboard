...
SELECT
    ...
    (SELECT GROUP_CONCAT(t.name, ', ')
      ...
      WHERE it.itemID = ann.itemID) AS Annotation_Tags,
        ...
        for (const [title, group] of Object.entries(grouped)) {{
            ...
            catOrder.forEach(cat => {{
                ...
                items.forEach(n => {{
                    ...
                    html += `<div class="note-card" style="border-left-color:${{color}}" data-note='${{JSON.stringify(noteData)}}'>`;
                    ...
                    if (n.Annotation_Tags) {{
                        const noteTags = n.Annotation_Tags.split(',').map(t => t.trim());
