...
body {{font-family:'Segoe UI',system-ui,sans-serif;background:#f0f2f5;margin:0;display:flex;height:100vh;overflow:hidden;color:#2c3e50}}
...
.filter-search-box::placeholder {{color:#999;font-style:italic}}
...
#tag-filter option {{padding:5px 8px;border-radius:3px;margin-bottom:2px}}
#tag-filter option:checked {{background:#2196f3;color:#fff;font-weight:600}}
.clear-btn,.export-btn {{margin-top:8px;width:100%;padding:8px;background:#3498db;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:700;font-size:11px}}
...
.copy-citation-btn {{position:absolute;top:20px;right:20px;padding:6px 12px;background:#3498db;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:11px;font-weight:600;transition:.2s}}
...
.note-card {{background:#fff;border:1px solid #e8e8e8;border-left:5px solid #ccc;border-radius:7px;padding:13px;margin-bottom:12px;box-shadow:0 1px 3px rgba(0,0,0,.025);position:relative}}
...
function exportFilteredNotes() {{
    ...
    if (selectedTags.size > 0) {{
        exportText += `Tags: ${{Array.from(selectedTags).join(', ')}}\\n`;
    }}
